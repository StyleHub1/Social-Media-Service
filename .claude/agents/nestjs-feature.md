...
| After generating all code | `/tdd-workflow` — ensure unit tests follow red-green-refactor and cover all branches |
| After generating all code | `/typescript-reviewer` — type safety, async correctness, NestJS idioms |

## Your Task

When given a feature description:
1. Invoke `/nestjs-patterns` and `/backend-patterns` first to align on conventions
...
6. Invoke `/typescript-reviewer` on generated code before finishing
